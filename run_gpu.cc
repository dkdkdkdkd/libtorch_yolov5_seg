...
int main() {
    // Load the YOLO model
    torch::jit::script::Module model = torch::jit::load("../yolov5s-seg-gpu.torchscript");
    model.to(at::kCUDA);
    // Open the webcam
    ...
    while (true) {
        ...
        tensor_image = tensor_image.to(at::kFloat).div(255).to(at::kCUDA);
        ...
        person_mask = person_mask.to(torch::kCPU);
